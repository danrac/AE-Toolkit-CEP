...
Composition Formats now offers **Create General** and **Create Custom** dialogs. General formats retain dimensions and a variable list of mattes/guides. Custom checkers use a native `.aepx` XML snapshot with JSON metadata, rather than reconstructing effects and expressions from a new interchange format.

1. Prepare a dedicated checker project containing the selected templates and their dependencies. Save it as `.aep` or `.aepx` before capture. The project must have no unsaved changes.
...
3. Choose a Template library folder (local or a mounted shared folder), then **Create Custom → Use selected comps**. Multiple selected checker comps become separate presets. Capture exports native XML and then restores the original project file path. The current project is never closed or reduced.
...
A package contains `project.aepx`, `template.json`, and copied media. The manifest uses package-relative paths. Failed captures never publish a valid manifest; failed checker creation removes newly imported items. The snapshot contains the whole dedicated template project, including expression dependencies, so each generated checker also imports that package’s other templates. Fonts and third-party plugins must be installed separately.

Native XML media references are resolved to the mounted library at import time, including layered PSD/AI sources and copied numbered image sequences. Layer import settings remain native: sources are not replaced with flattened footage. Missing media or an unavailable library stops the operation. Legacy development packages using binary `.aep` should be recaptured for layered-media portability. Custom creation preserves template timing; it does not apply the General checker’s held-frame setting.

Validation includes strict ES3 parsing before tests and packaging, model tests, mocked native-project integration, and a separate After Effects 26.5 instance. Native tests captured two templates and created two checkers, verified nested placeholders and Job text, and imported a relocated package with the original layered PSD unavailable. The original project file path and source template were preserved. The exact reference artwork was unavailable; Windows native import has not been tested. Fonts/plugins must be available on each workstation, and expressions that dynamically reference external comps or absolute paths still require a self-contained template.
