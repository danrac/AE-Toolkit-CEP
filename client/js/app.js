(function () {
    ...
    function startCompPresetDraft(preset) { preset = preset || { id: "", name: "", width: "", height: "", assets: {} }; compPresetDraft = { id: preset.id || "", name: preset.name || "", formatCode: preset.id ? store.compFormatCode(preset) : "", width: preset.width || "", height: preset.height || "", assets: copy(preset.assets || {}) }; }
    function renderCompPresetForm() { var select = byId("saved-comp-preset"); select.innerHTML = ""; compPresets().forEach(function (preset) { var option = document.createElement("option"); option.value = preset.id; option.textContent = preset.name + " · " + preset.width + " × " + preset.height; select.appendChild(option); }); if (compPresetDraft.id) select.value = compPresetDraft.id; byId("saved-comp-preset-name").value = compPresetDraft.name; byId("saved-comp-preset-code").value = compPresetDraft.formatCode; byId("saved-comp-preset-width").value = compPresetDraft.width; byId("saved-comp-preset-height").value = compPresetDraft.height; byId("saved-comp-preset-matte").value = compPresetDraft.assets.matte || ""; byId("saved-comp-preset-chart-one").value = compPresetDraft.assets.chartOne || ""; byId("saved-comp-preset-chart-two").value = compPresetDraft.assets.chartTwo || ""; }
