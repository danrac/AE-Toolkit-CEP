...
const data=JSON.parse(a.aetoolkitCepLoadState()); data.namingPresets=[{id:'studio',name:'Studio'}];data.compPresets=[{id:'square',assets:{matte:'library:guide-assets/square/matte.png'}}];data.projects=[{id:'job',root:'/Volumes/Jobs/Job',templateId:'default-motion'}];
assert.equal(a.aetoolkitCepSaveState(JSON.stringify(data)),'OK');
const loaded=JSON.parse(b.aetoolkitCepLoadState());assert.equal(loaded.namingPresets[0].name,'Studio');assert.equal(loaded.compPresets[0].id,'square');assert.equal(loaded.projects[0].id,'job');assert.equal(loaded.libraryRevision,1);
